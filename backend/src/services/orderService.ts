...
import { Order, OrderItemInput, SellerOrder, PaymentInfo } from '@mytypes/orderTypes';
...
export class OrderService {
  ...
  async createOrder(
    ...
    const shipping = await this.shippingService.getDefaultShippingAddress(userId); // 🔑 Get shipping info
    
  if (!shipping || !shipping._id) {
    throw AppError.notFound("Shipping address not found for user.");
  }
  const shippingId = shipping._id.toString();
    const orderItems = await Promise.all(
      (cart.items as CartItem[]).map(async (item) => {
        ...
        return {
          ...
          sellerStatus: OrderItemStatus.PENDING,
        };
      })
    );
  ...
  await this.auditService.logAudit({
    ...
    return { message: "Order created successfully", _id: newOrder._id.toString(), };
  }
  ...
  async getOrderBySellerId(sellerId: string): Promise<{ message: string } | SellerOrder[] > {
    const orders = await this.orderRepository.getOrderBySellerId(sellerId);
    return orders;
  }
  ...
  private updateOverallOrderStatus(itemStatuses: OrderItemStatus[]): Status {
    ...
    if (someDelivered) return Status.PARTIALLY_DELIVERED;
    if (someShipped && !allShippedOrDelivered) return Status.PARTIALLY_SHIPPED;
  ...
  async updateOrderItemStatus(
    ...
    const oldStatus = itemToUpdate.sellerStatus;
    if (newStatus === OrderItemStatus.CANCELLED && itemToUpdate.sellerStatus !== OrderItemStatus.PENDING) {
    ...
    const updatedItems = order.items.map(item => {
      if (item._id.toString() === itemId) {
        return { ...item, sellerStatus: newStatus };
      }
      return item;
    });
    ...
    const itemStatuses = updatedItems.map(item => item.sellerStatus);
  ...
  async cancelOrder(orderId: string, userId: string, req?: Request): Promise<{ message: string }> {
    ...
    for (const item of order.items) {
      if (item.sellerStatus !== OrderItemStatus.CANCELLED) {
