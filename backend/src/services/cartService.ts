...
export class CartService {
  ...
  async createCart(productId: string, quantity: number, userId: string, req?: Request): Promise<{ message: string }> {
    ...
    if (!userCart) {
      const newCart = await this.cartRepository.add({
        userId,
        items: [{ productId: product._id, productName: product.name, quantity, priceAtAddition: product.price, sellerId: product.sellerId }],
      });
      ...
      userCart.items.push({ productId: product._id, productName: product.name, quantity, priceAtAddition: product.price, sellerId: product.sellerId });
  ...
  async updateItemQuantity(userId: string, productId: string, quantity: number, req?: Request): Promise<{ message: string }> {
    ...
    const cart = await this.cartRepository.findRawCartByUserId(userId);
    logger.debug("Updating cart quantity", { userId, productId, quantity });
    if (!cart) {
      logger.warn("Cart not found");
      throw AppError.notFound("Cart not found", userId);
    }
    logger.debug("Cart items product IDs", {
        cartItems: cart.items.map(item => ({
            storedId: item.productId,
            type: typeof item.productId,
            toString: item.productId.toString()
        })),
        inputProductId: productId,
        typeOfInput: typeof productId
    });

    // Simplified comparison that matches removeFromCart
