...
export class OrderController {
    ...
    async getById(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
        ...
        try {
            ...
            logger.info(`Orders fetched successfully`, { orderId });
            handleSuccess(res, result);
        } catch(error) {
            handleError(next, error);
        }
    ...
    async updateOrderItemStatus(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
        ...
        const newStatus = req.body.sellerStatus;
        try {
    ...
    async createUserOrder(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
        ...
        try {
            ...
            res.status(400).json({ 
                success: false, 
                message: 'Invalid payment method. Must be stripe, paypal, or cod.' 
            });
            }
            const result = await this.orderService.createOrder(userId, paymentMethod, req);
