...
import { Trash2, Truck, PackageCheck, X, User, CheckCircle, Clock, CreditCard,HandCoins } from "lucide-react";
...
const AdminOrderTable = () => {
    ...
    const handleSearch = () => {
        if (searchTerm.trim()) {
            navigate(`/admin/order?search=${encodeURIComponent(searchTerm.trim())}`);
            setSearchTerm('');
        } else {
...
},
{
    ...
    cell: ({ row }) => (
        <div className="flex gap-2">
            {['Pending', 'Partially Shipped'].includes(row.original.status) && (
                <Button
                    variant="outline"
                    size="sm"
                    aria-label={`Cancel order #${row.original._id ? row.original._id.substring(0, 8) : 'N/A'}`}
                >
                    Cancel
                </Button>
            )}
            <Button
                variant="ghost"
                size="icon"
                aria-label={`Delete order #${row.original._id ? row.original._id.substring(0, 8) : 'N/A'}`}
            >
